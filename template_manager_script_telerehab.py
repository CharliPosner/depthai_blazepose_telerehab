...
def send_result(type, lm_score=0, rect_center_x=0, rect_center_y=0, rect_size=0, rotation=0, lms=0, lms_world=0, xyz_ref=0, xyz=0, xyz_zone=0, exercise_output=None, rating_output=None):
    # type : 0, 1 or 2
    ...
    result = dict([("type", type), ("lm_score", lm_score), ("rotation", rotation),
            ...
            ##################################################################################################################################
            ("exercise_output", exercise_output), ("rating_output", rating_output)])
            ##################################################################################################################################
...
########################################################################################################################
    ...
    rehab_result = node.io['from_rehab_nn'].tryGet()
    
    if rehab_result:
        exercise_output = rehab_result.getLayerFp16("exercise_output")        
        rating_output = rehab_result.getLayerFp16("rating_output")
        
    else: 
        exercise_output = None
        rating_output = None
...
########################################################################################################################
    ...
    if lm_score > ${_lm_score_thresh}:
        ...
        # Send result to host
        send_result(send_new_frame_to_branch, lm_score, sqn_rr_center_x, sqn_rr_center_y, sqn_rr_size, rotation, lms, lms_world, xyz_ref, xyz, xyz_zone, exercise_output, rating_output)
